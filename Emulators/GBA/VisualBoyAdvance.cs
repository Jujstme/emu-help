...
namespace LiveSplit.EMUHELP.GBA
{
    internal class VisualBoyAdvance : GBABase
    {
        ...
        internal VisualBoyAdvance(HelperBase helper) : base(helper)
        {
            if (Helper.Game.Is64Bit())
            {
                ...
                {
                    new(2, "83 3D ?? ?? ?? ?? 00 74 ?? 80 3D ?? ?? ?? ?? 00 75 ?? 66") { OnFound = (p, s, addr) => addr + 0x4 + p.ReadValue<int>(addr) + 0x1 },
                    new(3, "48 8B 15 ?? ?? ?? ?? 31 C0 8B 12 85 D2 74 ?? 48") { OnFound = (p, s, addr) => p.ReadPointer(addr + 0x4 + p.ReadValue<int>(addr)) },
                };
            ...
            else
            {
                ...
                if (!cached_ewram_pointer.IsZero())
                {
                    ...
                    SigScanTarget[] isEmulating =
                    {
                        new(2, "83 3D ?? ?? ?? ?? 00 74 ?? 80 3D ?? ?? ?? ?? 00 75 ?? 66") { OnFound = (p, s, addr) => p.ReadPointer(addr) },
                        new(2, "8B 15 ?? ?? ?? ?? 31 C0 85 D2 74 ?? 0F") { OnFound = (p, s, addr) => p.ReadPointer(addr) },
                    };
